...
in [nextflow/conf/defaults.config](nextflow/conf/defaults.config) must equal the version
derived from the pushed ref (`<version>` for `v*` tags and `release/*` branches,
`<version>-SNAPSHOT` for `snapshot/*` branches). On `master` the value stays `latest` unless
a release or snapshot build is intended.
...
1. **Local git hook** - a `pre-push` hook refuses to push a `v*` tag or a `release/*` /
   `snapshot/*` branch whose commit does not carry the matching `defaultUnifireVersion`.
...
1. Update `nextflow/conf/defaults.config`: `defaultUnifireVersion = "<major>.<minor>.<patch>-SNAPSHOT"`.
